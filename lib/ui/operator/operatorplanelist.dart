...
class _OperatorPlaneListPage extends State<OperatorPlaneListPage> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(
          title: Text(operator.user.name + "'s " + "planes"),
          leading: IconButton(
        ...
        body: RefreshIndicator(
          key: refreshKey,
          onRefresh: () async {
            var tmp = await getPlanesHandler(operator.id);
            setState(() {
              planeList = tmp;
            });
          },
          child: Padding(
            ...
            child: FutureBuilder<List<Plane>>(
              future: getPlanesHandler(operator.id),
              builder:
                  (BuildContext context, AsyncSnapshot<List<Plane>> snapshot) {
                Widget child;
                if (snapshot.hasData) {
                  print("there");
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => OperatorAddPlane(operator)));
          },
        ),
      ),
    );
...
class PlaneCard extends StatelessWidget {
  const PlaneCard(this.refreshKey, this.plane, this.operator, {Key key})
      : super(key: key);
        ...
        child: GestureDetector(
          onTap: () {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => OperatorPlaneInfo(operator, plane)));
          },
          child: Column(
